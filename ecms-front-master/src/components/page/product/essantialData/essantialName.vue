    ...
    <div class="container">
        ...
        <div class="materialList">
            <div class="handle-box" style="margin-top: 10px">
                <span class="text">物料名称列表</span>
                <el-button round type="primary" @click="add" style="margin-bottom: 10px">新增物料名称</el-button>
            </div>
            <div class="materialTable">
                <el-table v-loading="loading" :data="tables" border style="width:120%">
                    <el-table-column label="序号" prop="id" align="center" sortable>
                        <template slot-scope="scope">
                            {{scope.$index+1}}
                        </template>
                    </el-table-column>
                    <el-table-column label="物料名称编号" prop="number" align="center"></el-table-column>
                    <el-table-column label="物料名称" prop="type" align="center"></el-table-column>
        ...
        <el-dialog title="新增物料名称" @close="closeing" :visible.sync="editVisible" width="24%">
            <el-form ref="addForm" :rules="rules" :model="addForm" label-width="120px">
                <el-form-item label="物料类型编号:" prop="number">
                    <el-input style="width: 215px;" :maxlength="2" v-model="addForm.number"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        data () {
            return {
                ...
                rules: {
                    number: [
                        { required: true, message: '请输入名称编号', trigger: 'blur' }
                    ],
                    type: [
        ...
        methods: {
            ...
            getData(){
                this.search.productTypeId = this.$route.query.row.id;
                this.parentForm = this.$route.query.row
                if (this.parentForm.status == 0){
                    this.parentForm.status = '停用'
                } else {
                    this.parentForm.status = '启用'
                }
                this.$http.post('/productName/searchProductNamePager', this.search).then((res) => {
            ...
            updateStatus(index, row) {
                this.$http.post('/productName/status/update', {
                    id: row.id
                }).then((res) => {
                    if (res != undefined && res.data.code == 1000) {
            ...
            insert(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.addForm.productTypeId = this.$route.query.row.id;
                        this.$http.post('/productName/insertProductName', this.addForm).then((res) => {
                            if (res != undefined && res.data.code == 1000) {
                                this.getData();
        ...
        watch: {
            '$route' (to, from) {
                if (to.path == "/essantialName" && this.$route.query.works != 1) {
                    this.getData();
                }
            }
        }
