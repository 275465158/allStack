        ...
        methods: {
            ...
            getData() {
                ...
                if(this.$route.query.repertoryId != null && this.$route.query.repertoryId != undefined){
                    this.$http.post("/supplier/supplierList", {materialId:this.$route.query.row.materialId}).then(res => {
                        if (res != undefined && res.data.code == 1000) {
                            this.supplierList = res.data.data;
                            this.pages = res.data.data.pages;
                        }
                    });
                    this.$http.post("/storage/getJson", { repertoryId: this.$route.query.repertoryId }).then(res => {
                        if (res != undefined && res.data.code == 1000) {
            ...
            goBack() {
                this.$router.push({
                    ...
                    query: { repertoryId: this.$route.query.repertoryId, closeFlag: 1 }
                });
            },
        },
        watch: {
            '$route' (to, inboundForm) {
                if (to.path == '/materialInboundList') {
                    ...
                    if (this.search.repertoryId != newId && this.inboundForm.materialId != newMaterialId) {
                        this.search.repertoryId = newId;
                        this.inboundForm.materialId = newMaterialId;
                        this.getData();
                    }
                }
            }
