    ...
    <div class="table">
        <div class="container">
            ...
            <el-table :data="tables" border stripe style="width: 100%">
                ...
                <el-table-column align="center" prop="name" label="付款方式"/>
...
<script>
    export default {
        ...
        methods: {
            ...
            getData() {
                this.$http.post("/sellerPayType/searchPager",this.search).then((res) => {
            ...
            updateStatus(index, row) {
                this.$http.post('/sellerPayType/status/update', {
                    id: row.id
                }).then((res) => {
                    if (res != undefined && res.data.code == 1000) {
            ...
            insert(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$http.post('/sellerPayType/insertPayType', this.form).then((res) => {
                            if (res != undefined && res.data.code == 1000) {
                                this.getData();
