<template>
    <div class="table">
        ...
        <!-- 编辑弹出框 -->
        <el-dialog title="新增付款方式" @close="closeing" :visible.sync="editVisible" width="30%">
            <el-form ref="form" :rules="rules" :model="form" label-width="100px">
                <el-form-item label="付款方式:"  prop="paymentName">
                    <el-input v-model="form.paymentName"></el-input>
                </el-form-item>
...
export default {
  data() {
    return {
      tableData: [],
      url: "/purchase/payment/type/list",
      editVisible: false,
        rules: {
            paymentName: [
  ...
  methods: {
    ...
    updateStatus(index, row) {
      this.$http
        .post("/purchase/payment/type/status/update", {
          id: row.id,
          status: row.status
        })
        .then(res => {
    ...
    // 保存编辑
      insert(formName) {
          this.$refs[formName].validate((valid) => {
              if (valid) {
                  this.$http.post('/purchase/payment/type/insert', this.form).then((res) => {
