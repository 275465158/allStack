<template>
    <div class="table">
        ...
        <!-- 编辑弹出框 -->
        <el-dialog title="新增送货方式" @close="closeing" :visible.sync="editVisible" width="30%">
            <el-form ref="form" :rules="rules" :model="form" label-width="100px">
                <el-form-item label="送货方式:" prop="methodName">
...
export default {
  data() {
    return {
      tableData: [],
      url: "/purchase/shipping/method/list",
      editVisible: false,
        rules: {
            methodName: [
  ...
  methods: {
    ...
    updateStatus(index, row) {
      this.$http
        .post("/purchase/shipping/method/status/update", {
          id: row.id,
          status: row.status
        })
        .then(res => {
    ...
    // 保存编辑
      insert(formName) {
          this.$refs[formName].validate((valid) => {
              if (valid) {
                  this.$http.post('/purchase/shipping/method/insert', this.form).then((res) => {
